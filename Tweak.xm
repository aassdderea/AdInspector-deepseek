...
// ==================== 全局配置 ====================
static NSArray *s_tapConfigs = nil;      // 点击配置数组
static NSTimer *s_executeTimer = nil;    // 执行定时器
static BOOL s_isExecuting = NO;          // 是否正在执行
...
static void showToast(NSString *m) {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        l.text = m; l.textColor = [UIColor whiteColor]; l.font = [UIFont boldSystemFontOfSize:14];
        l.numberOfLines = 0; l.textAlignment = NSTextAlignmentCenter; [t addSubview:l];
...
static void iokitTap(CGFloat x, CGFloat y) {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.05 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        IOHIDEventRef upEvent = IOHIDEventCreateDigitizerFingerEvent(
            kCFAllocatorDefault,
            mach_absolute_time(),
            0, 2, 0x01, NO, NO,  // touch = NO (抬起)
            px, py, 0, 1.0, 0
        );
        if (upEvent) {
            IOHIDEventSystemClientRef c = IOHIDEventSystemClientCreate(kCFAllocatorDefault);
            if (c) {
                IOHIDEventSystemClientDispatchEvent(c, upEvent);
                CFRelease(c);
            }
            CFRelease(upEvent);
        }
        [[TapControllerPanel shared] showLog:[NSString stringWithFormat:@"👆 点击 (%.0f, %.0f)\n", x, y]];
    });
}
...
static void executeTapSequence(NSArray *configs, NSUInteger index) {
    if (index >= configs.count) {
        s_isExecuting = NO;
        showToast(@"✅ 全部点击完成");
        [[TapControllerPanel shared] showLog:@"✅ 全部完成\n"];
        return;
    }
    ...
    if (parts.count < 2) {
        executeTapSequence(configs, index + 1);
        return;
    }
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event {
    ...
    while (hit && (id)hit != (id)self.panel) {
        if (hit.tag >= 2001 && hit.tag <= 2020) return hit;
        hit = hit.superview;
    }
    return nil;
}
...
+ (instancetype)shared {
    static TapControllerPanel *i = nil;
    static dispatch_once_t t;
    dispatch_once(&t, ^{
        i = [[TapControllerPanel alloc] initWithFrame:CGRectMake(5, 180, [UIScreen mainScreen].bounds.size.width - 10, 300)];
    });
    return i;
}
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        _configField = [[UITextField alloc] initWithFrame:CGRectMake(95, 32, self.bounds.size.width - 110, 26)];
        _configField.borderStyle = UITextBorderStyleRoundedRect;
        _configField.backgroundColor = [UIColor darkGrayColor];
        _configField.textColor = [UIColor whiteColor];
        _configField.font = [UIFont systemFontOfSize:12];
        _configField.placeholder = @"x:y:秒|x2:y2:秒|...";
        _configField.tag = 2011;
        _configField.delegate = self;
        [self addSubview:_configField];
        
        // 预设按钮
        UIButton *p1 = [UIButton buttonWithType:UIButtonTypeSystem];
        p1.frame = CGRectMake(12, 66, 80, 26);
        [p1 setTitle:@"预设:左上角" forState:UIControlStateNormal];
        [p1 setTitleColor:[UIColor cyanColor] forState:UIControlStateNormal];
        p1.titleLabel.font = [UIFont systemFontOfSize:10];
        p1.tag = 2016;
        [p1 addTarget:self action:@selector(preset1) forControlEvents:UIControlEventTouchUpInside];
        [self addSubview:p1];
        
        UIButton *p2 = [UIButton buttonWithType:UIButtonTypeSystem];
        p2.frame = CGRectMake(100, 66, 80, 26);
        [p2 setTitle:@"预设:右下角" forState:UIControlStateNormal];
        [p2 setTitleColor:[UIColor cyanColor] forState:UIControlStateNormal];
        p2.titleLabel.font = [UIFont systemFontOfSize:10];
        p2.tag = 2017;
        [p2 addTarget:self action:@selector(preset2) forControlEvents:UIControlEventTouchUpInside];
        [self addSubview:p2];
        
        UIButton *p3 = [UIButton buttonWithType:UIButtonTypeSystem];
        p3.frame = CGRectMake(188, 66, 80, 26);
        [p3 setTitle:@"预设:屏幕中心" forState:UIControlStateNormal];
        [p3 setTitleColor:[UIColor cyanColor] forState:UIControlStateNormal];
        p3.titleLabel.font = [UIFont systemFontOfSize:10];
        p3.tag = 2018;
        [p3 addTarget:self action:@selector(preset3) forControlEvents:UIControlEventTouchUpInside];
        [self addSubview:p3];
        
        // 执行按钮
        UIButton *execBtn = [UIButton buttonWithType:UIButtonTypeSystem];
        execBtn.frame = CGRectMake(12, 100, 100, 36);
        [execBtn setTitle:@"⚡立即执行" forState:UIControlStateNormal];
        [execBtn setTitleColor:[UIColor systemGreenColor] forState:UIControlStateNormal];
        execBtn.titleLabel.font = [UIFont boldSystemFontOfSize:14];
        execBtn.tag = 2014;
        [execBtn addTarget:self action:@selector(executeTaps) forControlEvents:UIControlEventTouchUpInside];
        ...
        clearLogBtn.frame = CGRectMake(self.bounds.size.width - 180, 3, 40, 30);
        [clearLogBtn setTitle:@"清屏" forState:UIControlStateNormal];
        [clearLogBtn setTitleColor:[UIColor grayColor] forState:UIControlStateNormal];
        clearLogBtn.titleLabel.font = [UIFont systemFontOfSize:11];
        clearLogBtn.tag = 2031;
...
- (void)preset1 {
    self.configField.text = @"50:100:0";
    [self showLog:@"📌 预设: 左上角 (50,100)\n"];
}

- (void)preset2 {
    CGFloat w = [UIScreen mainScreen].bounds.size.width;
    CGFloat h = [UIScreen mainScreen].bounds.size.height;
    self.configField.text = [NSString stringWithFormat:@"%.0f:%.0f:0", w - 50, h - 100];
    [self showLog:@"📌 预设: 右下角\n"];
}
...
- (void)preset3 {
    CGFloat w = [UIScreen mainScreen].bounds.size.width;
    CGFloat h = [UIScreen mainScreen].bounds.size.height;
    self.configField.text = [NSString stringWithFormat:@"%.0f:%.0f:0", w / 2, h / 2];
    [self showLog:@"📌 预设: 屏幕中心\n"];
}

- (void)executeTaps {
    [self.configField resignFirstResponder];
    NSString *raw = self.configField.text;
    if (!raw.length) {
        showToast(@"⚠️ 请填写点击序列");
        return;
    }
    s_tapConfigs = [raw componentsSeparatedByString:@"|"];
    if (s_tapConfigs.count == 0) {
        showToast(@"⚠️ 格式: x:y:秒|x2:y2:秒");
        return;
    }
    s_isExecuting = YES;
    [self showLog:[NSString stringWithFormat:@"\n🖐 开始执行 %lu 步点击\n", (unsigned long)s_tapConfigs.count]];
    showToast([NSString stringWithFormat:@"🖐 开始 %lu 步", (unsigned long)s_tapConfigs.count]);
    executeTapSequence(s_tapConfigs, 0);
}
...
- (void)forceShow {
    if (!s_tapWindow) {
        ...
        if (as) {
            s_tapWindow = [[TapControllerWindow alloc] initWithFrame:as.coordinateSpace.bounds];
            s_tapWindow.windowScene = as;
            [s_tapWindow addSubview:self];
            self.frame = CGRectMake(5, 180, s_tapWindow.bounds.size.width - 10, 300);
            s_tapWindow.panel = self;
            s_tapWindow.hidden = NO;
        }
    } else {
        if (!self.superview) {
            [s_tapWindow addSubview:self];
        ...
        s_tapWindow.hidden = NO;
        s_tapWindow.alpha = 1.0;
        [s_tapWindow bringSubviewToFront:self];
    }
    self.hidden = NO;
    self.alpha = 1.0;
    showToast(@"👆 面板已呼出");
...
- (void)showLog:(NSString *)log {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (self.logBuffer.length > 5000) [self.logBuffer deleteCharactersInRange:NSMakeRange(0, self.logBuffer.length - 5000)];
...
%ctor {
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        if (as) {
            ...
            s_tapWindow.windowScene = as;
            TapControllerPanel *p = [TapControllerPanel shared];
            p.frame = CGRectMake(5, 180, s_tapWindow.bounds.size.width - 10, 300);
            p.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleBottomMargin;
            [s_tapWindow addSubview:p];
            s_tapWindow.panel = p;
            s_tapWindow.hidden = NO;
        }
        showToast(@"🖐 IOKit 触摸模拟器已激活");
    });
}
#pragma clang diagnostic pop
