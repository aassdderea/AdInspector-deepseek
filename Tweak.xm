...
static BOOL isFlexingAvailable(void) {
    for (UIWindow *w in getAllWindows()) {
        ...
        if ([cn isEqualToString:@"FLEXWindow"]||[cn isEqualToString:@"FLEXExplorerWindow"]||[cn isEqualToString:@"FLEXManagerWindow"]||[cn isEqualToString:@"FLEXOverlayWindow"]) return YES;
    }
    return NO;
}
static void raiseFlexingWindow(void) {
    for (UIWindow *w in getAllWindows()) {
        NSString *cn = NSStringFromClass([w class]);
        if ([cn isEqualToString:@"FLEXWindow"]||[cn isEqualToString:@"FLEXExplorerWindow"]||[cn isEqualToString:@"FLEXManagerWindow"]||[cn isEqualToString:@"FLEXOverlayWindow"]) {
...
static void recordMethodCall(NSString *name) {
    ...
    if ([name hasPrefix:@"set"]||[name hasPrefix:@"log"]||[name containsString:@"videoPlayer"]||[name isEqualToString:@"adModel"]||[name isEqualToString:@"adConfig"]||[name isEqualToString:@"delegate"]||[name isEqualToString:@"rootView"]||[name isEqualToString:@"gdm"]||[name hasPrefix:@"init"]||[name hasPrefix:@"."]||[name hasPrefix:@"_"]) return;
    @synchronized(s_trackedMethods) { [s_trackedMethods addObject:@{@"method":name, @"time":@([[NSDate date] timeIntervalSinceDate:s_trackStartTime])}]; }
}
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event { UIView *hit = [super hitTest:point withEvent:event]; if (hit == self || (id)hit == (id)self.panel) return nil; while (hit && hit != self.panel) { if (hit.tag >= 1001 && hit.tag <= 1020) return hit; hit = hit.superview; } return nil; }
...
static BOOL isSkipText(NSString *t){if(!t||t.length==0)return NO;for(NSString *k in @[@"跳过",@"广告",@"关闭",@"×",@"x",@"X",@"close",@"skip"]){if([t rangeOfString:k options:NSCaseInsensitiveSearch].location!=NSNotFound&&t.length<=15)return YES;}return NO;}
...
static void analyzeTouchView(UIView *v, CGPoint pt) {
    if (!v) return;
    if ([v isDescendantOfView:[AdInspectorPanel shared]] || [NSStringFromClass([v.window class]) isEqualToString:@"AdInspectorWindow"] || (v.tag >= 1001 && v.tag <= 1020)) return;
    ...
    @try {
        ...
        while (cur && d < 15) {
            ...
            if ([cur isKindOfClass:[UIButton class]]) { NSString *t = [(UIButton *)cur titleForState:UIControlStateNormal]; if (t.length) [tg addObject:[NSString stringWithFormat:@"\"%@\"", t]]; }
            if ([cur isKindOfClass:[UILabel class]]) { NSString *t = [(UILabel *)cur text] ?: [(UILabel *)cur attributedText].string; if (t.length > 20) t = [[t substringToIndex:20] stringByAppendingString:@"..."]; if (t.length) [tg addObject:[NSString stringWithFormat:@"\"%@\"", t]]; }
        ...
        while (cur && d < 8) {
                ...
                for (id tgt in c.allTargets) {
                    ...
                    for (int i = 0; i < 4; i++) {
                        ...
                        if (ac.count) {
                            ...
                            [ti addObject:@{@"viewClass": NSStringFromClass([cur class]), @"targetClass": NSStringFromClass([tgt class]), @"action": ac[0], @"event": @(ce[i])}];
                        }
                    }
                }
            }
            for (UIGestureRecognizer *gr in cur.gestureRecognizers) {
                ...
                if (!gti) [o appendString:@"    (无法提取)\n"];
            }
            cur = cur.superview; d++;
        }
        if (!fd) [o appendString:@"  (未检测到)\n"];
        ...
        r[@"hierarchyChain"] = ca;
        if (cc) r[@"containerClass"] = cc;
        if (wc) r[@"windowClass"] = wc;
        for (NSDictionary *info in ti) { if (info[@"event"] && [info[@"event"] unsignedIntegerValue] == UIControlEventTouchUpInside) { r[@"triggerType"] = @"controlEvent"; r[@"controlEvent"] = @(UIControlEventTouchUpInside); r[@"targetClass"] = info[@"targetClass"]; r[@"actionSelector"] = info[@"action"]; break; } }
        if (!r[@"triggerType"]) { for (NSDictionary *info in ti) { if (info[@"gestureClass"] && info[@"targetClass"] && info[@"action"]) { r[@"triggerType"] = @"gesture"; r[@"gestureClass"] = info[@"gestureClass"]; r[@"targetClass"] = info[@"targetClass"]; r[@"actionSelector"] = info[@"action"]; r[@"gestureViewClass"] = info[@"viewClass"]; break; } } }
        if (!r[@"triggerType"]) { cur = av; while (cur) { for (UIGestureRecognizer *gr in cur.gestureRecognizers) { r[@"triggerType"] = @"gesture"; r[@"gestureClass"] = NSStringFromClass([gr class]); r[@"gestureViewClass"] = NSStringFromClass([cur class]); break; } if (r[@"triggerType"]) break; cur = cur.superview; } }
        if (!r[@"triggerType"] && [av isKindOfClass:[UIControl class]]) { r[@"triggerType"] = @"controlEvent"; r[@"controlEvent"] = @(UIControlEventTouchUpInside); }
        if (r[@"triggerType"]) saveRule(r); else showToast(@"❌ 无法学习");
