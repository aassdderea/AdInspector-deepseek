...
- (void)toggleTracking:(UIButton *)sender {
    if (s_isTracking) { stopTracking(); [sender setTitle:@"▶开始追踪" forState:UIControlStateNormal]; [sender setTitleColor:[UIColor colorWithRed:1.0 green:0.5 blue:0.0 alpha:1.0] forState:UIControlStateNormal];
        if (s_trackedMethods.count > 0) { NSMutableString *o = [NSMutableString stringWithFormat:@"\n📊 追踪结果 (%lu个方法):\n", (unsigned long)s_trackedMethods.count]; [o appendString:@"─────────────────────────────\n"]; for (NSDictionary *e in s_trackedMethods) { [o appendFormat:@"  +%.2fs → %@\n", [e[@"time"] doubleValue], e[@"method"]]; } [o appendString:@"\n💡 关注非pauseTimer/onDestroy的GDTfunction方法\n"]; [self showLog:o]; }
        else { [self showLog:@"\n⚠️ 未捕获到方法调用\n"]; }
